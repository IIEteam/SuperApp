...
namespace OGL
{
    ...
    public class UzytkownikManager : UserManager<MyUser>
    {
        public UzytkownikManager(IUserStore<MyUser> store)
            : base(store)
        {
        }
        ...
        {
            var manager = new UzytkownikManager(new UserStore<MyUser>(context.Get<AdContext>()));
